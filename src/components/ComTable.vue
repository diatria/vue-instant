...
<template>
  <div>
    <!-- Toolbar -->
    <div class="flex justify-end gap-4 pb-4 border-b border-[#ebeef5]">
      <RouterLink v-if="props.buttonCreateUrl" :to="props.buttonCreateUrl()">
        <el-button :icon="Plus" type="primary">Tambah</el-button>
      </RouterLink>
      <el-button v-if="!$slots.buttonDelete && dataSelected.length" @click="dialogDeleteConfirmation = true" :icon="Delete" type="danger" class="!m-0">Hapus</el-button>
      <slot name="buttonDelete"></slot>
      <el-button class="!m-0">Filter</el-button>
    </div>

    <!-- Table -->
    <el-table v-loading="loading" :data="data" @selection-change="handleSelectionChange" style="width: 100%">
      <el-table-column type="selection" width="55" fixed="left" />
      <template v-for="(column, index) of props.columns" :key="index">
        <el-table-column v-if="!column.value && column.type !== 'slot'" :prop="column.field" :label="column.label"
        ...
        <!-- Slot -->
        <el-table-column v-if="column.type === 'slot'" :prop="column.field" :label="column.label" :width="column.width"
      ...
      <el-table-column width="55" fixed="right">
        <template #default="scope">
          <div class="hover:cursor-pointer hover:bg-slate-200 justify-center rounded flex items-center">
              ...
              <ul>
                ...
                <RouterLink v-if="typeof buttonViewUrl === 'function'" :to="buttonViewUrl(scope.row)" target="_blank">
                  <li class="flex items-center py-2 px-4 hover:cursor-pointer hover:bg-slate-100">
                    <el-icon>
                      <View />
                    </el-icon> <span class="ml-2">Lihat</span>
                  </li>
                </RouterLink>
                <!-- Action Button Edit -->
                <RouterLink v-if="typeof buttonEditUrl === 'function'" :to="buttonEditUrl(scope.row)">
                  <li class="flex items-center py-2 px-4 hover:cursor-pointer hover:bg-slate-100">
                    <el-icon>
                      <Edit />
                    </el-icon> <span class="ml-2">Ubah</span>
                  </li>
                </RouterLink>
    ...
    <div class="flex justify-end mt-4">
      <el-pagination v-model:page-size="pageSize" v-model:current-page="currentPage" :total="totalData"
        :page-sizes="[10, 25, 50, 75, 100]" @change="changePage" layout="sizes, total, prev, pager, next" />
    </div>
